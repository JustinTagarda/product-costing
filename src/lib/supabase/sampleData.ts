 ...
 * tables. Used to skip seeding for accounts that logged in before this
 * feature existed and have since added real data — sample data must never
 * be dropped in alongside it. Errs conservative: a failed count query is
 * treated as "has data" so a seed is never forced onto an account we
 * couldn't actually verify is empty.
 */
async function hasExistingAccountData(supabase: SupabaseClient, userId: string): Promise<boolean> {
